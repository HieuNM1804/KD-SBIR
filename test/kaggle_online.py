"""Build the offline Kaggle bundle for RSED.

Run once with Internet enabled. The output contains pinned source, offline Python
wheels, ViT-B/32 and DFN5B. Save the notebook output and attach it as an
input dataset to the offline GPU notebook.
"""
...
BUNDLE = WORKING / ("rsed_bundle_" + datetime.now().strftime("%Y%m%d_%H%M%S_%f")) / "offline_bundle"
...
required_paths = (
    ...
    project / "src" / "stroke_evidence.py",
    project / "src" / "stroke_evidence_cache.py",
    project / "src" / "stroke_evidence_diagnostics.py",
    project / "tests" / "test_stroke_evidence.py",
    project / "tests" / "test_rsed_commands.py",
    project / "test" / "RUN_ORDER.txt",
    project / "test" / "kaggle_main_baseline_train.ipy",
    project / "test" / "kaggle_rsed_prepare.ipy",
    project / "test" / "kaggle_rsed_train.ipy",
    project / "test" / "kaggle_rsed_attention_control.ipy",
    project / "test" / "kaggle_rsed_random_control.ipy",
...
print("=" * 70)
print("ONLINE RSED BUNDLE COMPLETE")
print("=" * 70)
print("Bundle:", BUNDLE)
print("Branch:", BRANCH)
