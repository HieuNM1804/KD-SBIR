"""Build a pinned offline Kaggle bundle for retrieval-vocabulary PromptKD.

Run this once in a Kaggle notebook with Internet enabled. Save the notebook
output and attach that output as an input to the offline GPU notebook.
"""
...
BUNDLE = (
    WORKING
    / ("promptkd_bundle_" + datetime.now(UTC).strftime("%Y%m%d_%H%M%S_%f"))
    / "offline_bundle"
)
...
# Kaggle provides the CUDA/PyTorch stack. Shipping a second copy can replace it
# with an incompatible build during offline installation.
for pattern in (
    "torch-*.whl",
    "torchvision-*.whl",
    "torchaudio-*.whl",
...
# Import the repository's vendored CLIP instead of a package named `clip`.
...
required_paths = (
    ...
    project / "src" / "photo_sketch_promptkd.py",
    project / "src" / "teacher_prompts.py",
    project / "src" / "train.py",
    project / "tests" / "test_photo_sketch_promptkd.py",
    project / "test" / "kaggle_promptkd_photo_sketch_train.ipy",
    project / "docs" / "photo_sketch_promptkd.md",
    dfn_target,
    student_target,
)
...
print("=" * 70)
print("ONLINE PROMPTKD BUNDLE COMPLETE")
print("=" * 70)
print("Bundle:", BUNDLE)
print("Branch:", BRANCH)
