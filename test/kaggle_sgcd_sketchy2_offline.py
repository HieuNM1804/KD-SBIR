"""Restore the Sketchy2 SGCD comparison on offline Kaggle; setup only."""
...
EXPECTED_COMMIT = "a021499ae64a969f0eec816e9c0a0abb57943537"
...
TEACHER_CACHE_NAME = "sketchy2_teacher_2ep.pt"
...
required_bundle_paths = (
    ...
    source_project / "test" / "kaggle_sgcd_sketchy2_baseline_where_effect.ipy",
    source_project / "test" / "kaggle_sgcd_sketchy2_online.py",
    source_project / "test" / "kaggle_sgcd_sketchy2_offline.py",
...
# Reuse only the prompt-tuned Sketchy2 teacher cache. The SGCD target cache is
# deliberately not restored: this comparison must execute the current audit.
...
if not teacher_cache_target.is_file():
    ...
    if cache_candidates:
        fingerprints = {
            (path.stat().st_size, file_sha256(path)) for path in cache_candidates
        }
        if len(fingerprints) != 1:
            raise RuntimeError(
                "Conflicting Sketchy2 teacher caches are attached:\n"
        ...
        print("Restored Sketchy2 teacher cache:", teacher_cache_target)
    else:
        print("No Sketchy2 teacher cache attached; baseline will create it.")
...
print()
print("=" * 70)
print("OFFLINE SKETCHY2 SGCD SETUP COMPLETE — READY TO RUN")
print("=" * 70)


os.chdir(WORKING_PROJECT)
print("Project:", WORKING_PROJECT)
print("Next: run test/kaggle_sgcd_sketchy2_baseline_where_effect.ipy as one cell")
print("The cell will not bypass a failed teacher-only gate.")
