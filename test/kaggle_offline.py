"""Restore mask-guided response KD on offline Kaggle; setup only."""
...
EXPECTED_BRANCH = "experiment/mask-guided-response-kd"
EXPECTED_COMMIT = "666d8b5c9cd2db3ef6f9e7a6ba72f9b2cf1fcf62"
EXPECTED_TASK = "mask_guided_response_kd"
...
if not matching_bundles:
    raise FileNotFoundError(
        "Cannot find the required mask-guided response KD bundle.\n\n"
...
required_bundle_paths = (
    ...
    source_project / "src" / "mask_guided.py",
    source_project / "src" / "mask_guided_cache.py",
    source_project / "src" / "mask_guided_diagnostics.py",
    source_project / "tests" / "test_mask_guided.py",
    source_project / "tests" / "test_mask_guided_cache.py",
    source_project / "test" / "kaggle_mask_prepare.ipy",
    source_project / "test" / "kaggle_mask_guided_train.ipy",
    source_project / "test" / "kaggle_mask_random_train.ipy",
    source_project / "test" / "kaggle_mask_embedding_train.ipy",
    source_project / "test" / "kaggle_main_baseline_train.ipy",
    source_project / "test" / "kaggle_mask_report.py",
    dfn_source,
    student_source,
)
missing_paths = [
...
import unittest
suite = unittest.defaultTestLoader.discover('tests', pattern='test_mask_guided*.py')
result = unittest.TextTestRunner(verbosity=2).run(suite)
if not result.wasSuccessful():
    raise SystemExit('Mask-guided response smoke test failed')
print('Mask alignment, response gradients, inference and cache tests: OK')
"""
subprocess.run(
...
print('First run test/kaggle_mask_prepare.ipy for supervision diagnostics.')
print('Controls: kaggle_mask_embedding_train.ipy, kaggle_mask_random_train.ipy.')
print('Method: kaggle_mask_guided_train.ipy; export: kaggle_mask_report.py.')
