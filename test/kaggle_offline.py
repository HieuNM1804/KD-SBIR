"""Restore RSED on offline Kaggle; setup only."""
...
if not matching_bundles:
    raise FileNotFoundError(
        "Cannot find the required RSED bundle.\n\n"
...
required_bundle_paths = (
    ...
    source_project / "src" / "stroke_evidence.py",
    source_project / "src" / "stroke_evidence_cache.py",
    source_project / "src" / "stroke_evidence_diagnostics.py",
    source_project / "tests" / "test_stroke_evidence.py",
    source_project / "tests" / "test_rsed_commands.py",
    source_project / "test" / "RUN_ORDER.txt",
    source_project / "test" / "kaggle_main_baseline_train.ipy",
    source_project / "test" / "kaggle_rsed_prepare.ipy",
    source_project / "test" / "kaggle_rsed_train.ipy",
    source_project / "test" / "kaggle_rsed_attention_control.ipy",
    source_project / "test" / "kaggle_rsed_random_control.ipy",
...
import unittest
suite = unittest.defaultTestLoader.discover('tests', pattern='test_stroke_evidence.py')
suite.addTests(unittest.defaultTestLoader.discover('tests', pattern='test_rsed_commands.py'))
result = unittest.TextTestRunner(verbosity=2).run(suite)
if not result.wasSuccessful():
    raise SystemExit('RSED smoke test failed')
print('RSED capture, ink graph, exact erasure, field losses, main preservation and commands: OK')
"""
subprocess.run(
...
print("Ready: python -m src.train with --retrieval_head rsed --lambda_rsed 1")
print("Run test/kaggle_rsed_prepare.ipy first; see test/RUN_ORDER.txt.")
