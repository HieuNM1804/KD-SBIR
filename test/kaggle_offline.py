"""Restore attention-anchor retrieval KD on offline Kaggle; setup only."""
...
EXPECTED_BRANCH = "experiment/attention-anchor-retrieval-kd"
EXPECTED_COMMIT = "1128b4789abe21b77eebbcc00612c7fcca552c67"
EXPECTED_TASK = "attention_anchor_retrieval_kd"
...
if not matching_bundles:
    raise FileNotFoundError(
        "Cannot find the required attention-anchor retrieval KD bundle.\n\n"
...
required_bundle_paths = (
    ...
    source_project / "src" / "attention_anchor.py",
    source_project / "src" / "attention_anchor_cache.py",
    source_project / "tests" / "test_attention_anchor.py",
    source_project / "tests" / "test_attention_anchor_cache.py",
    source_project / "test" / "kaggle_anchor_prepare.ipy",
    source_project / "test" / "kaggle_anchor_train.ipy",
...
import unittest
suite = unittest.defaultTestLoader.discover('tests', pattern='test_attention_anchor*.py')
result = unittest.TextTestRunner(verbosity=2).run(suite)
if not result.wasSuccessful():
    raise SystemExit('Attention-anchor retrieval smoke test failed')
print('Anchor capture, standalone gradients, inference and cache tests: OK')
"""
subprocess.run(
...
print('First: paste test/kaggle_anchor_prepare.ipy to evaluate the teacher descriptor.')
print('Then: test/kaggle_anchor_train.ipy; baseline: test/kaggle_main_baseline_train.ipy.')
