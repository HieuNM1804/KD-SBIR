...
for index, condition in enumerate(conditions):
    if cache_code != 0 or (index > 0 and return_codes.get("main") != 0):
        return_codes[condition["condition"]] = None
        (OUT / f"{condition['condition']}.log").write_text(
            "Not started because cache preparation or the matched main run failed.\n",
            encoding="utf-8",
        )
        continue
    command = [
...
cache_inventory = {
    ...
    "legacy_source_path": str(LEGACY_TEACHER_CACHE),
    "legacy_source_exists": LEGACY_TEACHER_CACHE.is_file(),
    "preparation_return_code": cache_code,
}
