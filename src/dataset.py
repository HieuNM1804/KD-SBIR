...
class TrainDataset(torch.utils.data.Dataset):
    def __init__(self, args):
        ...
        self.stroke_evidence_targets = None
    ...
    def set_stroke_evidence_targets(self, payload):
        expected = len(self.all_sketches_path)
        required = ("maps", "teacher_evidence", "teacher_masked", "confidence")
        if any(key not in payload or len(payload[key]) != expected for key in required):
            raise ValueError("Stroke-evidence cache has the wrong length or fields.")
        self.stroke_evidence_targets = {key: payload[key] for key in required}
    ...
    def __getitem__(self, sample_key):
        ...
        if self.stroke_evidence_targets is None:
            return result
        return result + ({
            key: value[index]
            for key, value in self.stroke_evidence_targets.items()
        },)
