...
def text_relational_kd_loss(
    ...
    """Match class-to-class text relations without requiring equal embedding dims."""

    def one_relation(student_text, teacher_text):
    ...
    return 0.5 * (
        one_relation(student_sketch_text, teacher_sketch_text)
        + one_relation(student_photo_text, teacher_photo_text)
    )
...
def loss_fn(args, features):
    ...
    if teacher_active and args.lambda_text_kd > 0:
        text_kd_loss = text_relational_kd_loss(
            student_sketch_text,
            student_photo_text,
            teacher_sketch_text,
            teacher_photo_text,
