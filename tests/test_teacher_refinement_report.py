...
class TeacherRefinementReportTests(unittest.TestCase):
    ...
    def _write(directory, seed, delta, semantic_weight=0.25):
        ...
        result = {
            "best_phase": "semantic_refine" if delta > 0 else "visual_bootstrap",
            "phase_a_acc1": 0.2,
            "phase_a_acc5": 0.4,
            "best_acc1": 0.2 + delta,
            "best_acc5": 0.4 + delta,
    ...
    def test_three_compatible_seeds_are_aggregated(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(summary["positive_acc1_runs"], 3)
            self.assertTrue(summary["all_acc1_positive"])
            self.assertAlmostEqual(summary["mean_delta_acc1"], 0.02)
