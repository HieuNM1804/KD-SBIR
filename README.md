# KD-SBIR: MetaCLIP ViT-H/14 Teacher

This branch is based directly on `main` at
`b2d50842f7831c9eb14f06ddb6cbe5bbd22255b6`. It replaces the DFN5B teacher
with the 224-pixel MetaCLIP ViT-H/14 checkpoint while leaving the ViT-B/32
student and distillation objectives unchanged. The teacher uses OpenCLIP model
`ViT-H-14`, pretrained tag `metaclip_fullcc`, and 1024-dimensional image/text
embeddings.
...
12, the teacher has 92,160 trainable prompt parameters.
...
!python -m src.train \
    ...
    --seed 42 \
    --exp_name metaclip_h14_teacher_visual_student_visual_only \
    --progress
```
